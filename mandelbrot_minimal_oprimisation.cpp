...
int DrawMandelbrot(RGBQUAD* window_buffer) {
    ...
    for (int y_cord = 0; y_cord < WINDOW_HEIGHT; y_cord++) {
        ...
        for (int x_cord = 0; x_cord < WINDOW_WIDTH; x_cord += 8, x0 += x_step * 8) {
            double x0_array[8] = {x0, x0 + x_step, x0 + 2*x_step, x0 + 3*x_step, x0 + 4*x_step, x0 + 5*x_step, x0 + 6*x_step, x0 + 7*x_step};
            ...
            for (int i = 0; i < 8; i++) {
                RGBQUAD new_color = {(BYTE) (N_array[i] % 2 * 255), (BYTE) (N_array[i] % 2 * 255), (BYTE) (N_array[i] % 2 * 255)};
